...
class moose(animal_base):
    def maybe_do_activity(self, structureContext):
        if structureContext.type != "enclosure":
            # moose actually can swim and dive long distances, but would generally prefer to live out of the water regardless
            self.happiness -= 4
        feeder_index = -1
        i = 0
        ...
        if random.random() >= 0.2:
            match random.choice(["sit", "sit", "charge", "vague_moose_noise"]):
                case "sit":
                ...
                case "vague_moose_noise":
                    # i have no idea what sound these things make
